...
@AllArgsConstructor
@EqualsAndHashCode
@TableName("t_pay_temp_order_history")
public class PayTempOrderHistory extends BaseDO<PayTempOrderHistory> implements Syncable {
    ...
    @Trans(type= TransType.SIMPLE,target = UcenterTollCollector.class,fields = "name",alias = "collector")
    ...
    public String getPayTimeStr() {
        if (CheckUtils.isNotEmpty(this.getPayTime())){
            Date date = DateUtils.parseStr(this.getPayTime().toString(), DateUtils.DATE_FULL_SS_NO_SP);
            return DateUtils.formartDate(date, DateUtils.DATETIME_PATTERN);
        }
        return this.payTime.toString();
    }
}
